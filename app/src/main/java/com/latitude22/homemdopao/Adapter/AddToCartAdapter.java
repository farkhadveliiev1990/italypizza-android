...
import com.latitude22.homemdopao.Fragments.AddToCartFragment;
...
public class AddToCartAdapter extends BaseAdapter {
    ...
    AddCart cartTable;
    Context Mcontext;
public AddToCartAdapter(ArrayList<AddCart> addCarts,Context Mcontext)
{
        ...
        try{
            if (addCarts.get(position).getProductImage()!=null&!addCarts.get(position).getProductImage().equals("")){
                ...
                Glide.with(Mcontext).load(addCarts.get(position).getProductImage())
                        ...
                        .into(cart_image);
              //  Picasso.with(Mcontext).load(addCarts.get(position).getProductImage()).error(R.drawable.bread_images).placeholder(R.drawable.bread_images).into(cart_image);
            }
